...
**Features:**
- Detects car presence via ToF sensor and drives LED color (red = arriving, green = present)
- Monitors air quality (IAQ index translated to human-readable French labels) with a smoke/fire alert derived from rapid temperature rise combined with elevated CO₂ equivalent
...
Pin assignments are documented as comments inside each YAML file (I2C, SPI, GPIO). Double-check the ESP32-C5 pinout before wiring, and avoid `GPIO5` on all boards in this repo, as it's used during boot.
